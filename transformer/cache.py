...
class AttentionCache:
    ...
    def keys_values(self, compute) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        # self-attention: compute K/V for only the newest tokens, append to the cache
    ...
    def attention_mask(self, mask, new_length: int, device: torch.device):
        if not self.growing:
            # cross-attention mask (source padding) doesn't depend on decoding position
            return mask
        # self-attention: causal mask over cached-plus-new positions; read `past`
        # BEFORE keys_values() below grows the cache with this step's new tokens
