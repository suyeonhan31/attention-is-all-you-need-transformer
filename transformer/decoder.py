...
class DecoderLayer(nn.Module):
    ...
    def forward(
        ...
        x = self.sublayers[0](
            x,
            lambda y: self.self_attn(y, y, y, tgt_mask, store_attention=store_attention),
        )  #(B, Lt, D) -> (B, Lt, D)

        # cross-attention: Q=y (decoder), K=V=memory (encoder output), masked by src_mask
        x = self.sublayers[1](
            x,
            lambda y: self.cross_attn(y, memory, memory, src_mask, store_attention=store_attention),
        )  #(B, Lt, D) -> (B, Lt, D)
...
class Decoder(nn.Module):
    ...
    def forward(
        ...
        for layer in self.layers:
            x = layer(x, memory, src_mask, tgt_mask, store_attention=store_attention)  # (B, Lt, D) -> (B, Lt, D)
        return x
